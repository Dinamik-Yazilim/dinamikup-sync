...
function post(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!data.category) return reject('category required')
      const categoryDoc = await dbModel.categories.findOne({ _id: data.category })
      if (!categoryDoc) return reject(`category not found`)
      const c = await dbModel.brands.countDocuments({ name: data.name })
      if (c > 0) return reject(`name already exists`)

      const newDoc = new dbModel.brands(data)
...
function put(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let doc = await dbModel.brands.findOne({ _id: req.params.param1 })
      if (!doc) return reject(`record not found`)

      doc = Object.assign(doc, data)
      if (!epValidateSync(doc, reject)) return
      if (await dbModel.brands.countDocuments({ name: doc.name, _id: { $ne: doc._id } }) > 0)
...
function deleteItem(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      dbModel.brands.removeOne(sessionDoc, { _id: req.params.param1 })
        .then(resolve)
        .catch(reject)
    } catch (err) {
      reject(err)
