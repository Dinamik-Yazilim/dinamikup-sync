...
function StorePage({ store, token, audioRef }: StorePageProps) {
  ...
  useEffect(() => { !busyItems && audioRef?.current && audioRef.current.play() }, [busyItems, audioRef])
  useEffect(() => { !busyFirms && audioRef?.current && audioRef.current.play() }, [busyFirms, audioRef])
  useEffect(() => { !busyStaff && audioRef?.current && audioRef.current.play() }, [busyStaff, audioRef])
  ...
  return (
    <div className="border rounded-md border-dashed px-4 py-2 flex flex-col gap-4 w-full min-h-40">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-2 items-end">
        ...
        <ProgressBar storeId={store._id} className="col-span-3" title="Stok Aktarimi" eventName="syncItems_progress" onProgress={e => setBusyItems(true)} onFinished={() => setBusyItems(false)} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-2 items-end">
        ...
        <ProgressBar storeId={store._id} className="col-span-3" title="Cari Aktarimi" eventName="syncFirms_progress" onProgress={e => setBusyFirms(true)} onFinished={() => setBusyFirms(false)} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-2 items-end">
        ...
        <ProgressBar storeId={store._id} className="col-span-3" title="Personel Aktarimi" eventName="syncStaff_progress" onProgress={e => setBusyStaff(true)} onFinished={() => setBusyStaff(false)} />
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-8 mt-6">
        {stores && stores.map(store => <StorePage key={store._id} store={store} token={token} audioRef={audioRef} />)}
      </div>
