...
export default function UserEditPage({ params }: Props) {
  ...
  const load = () => {
    setLoading(true)
    getItem(`/admin/members/${params.id}`, token)
      .then(result => {
        console.log(result)
  ...
  const save = () => {
    if (!member?._id) {
      postItem(`/admin/members`, token, member)
        .then(result => router.back())
        .catch(err => toast({ title: 'Hata', description: err || '', variant: 'destructive' }))
    } else {
      putItem(`/admin/members/${member?._id}`, token, member)
        .then(result => router.back())
        .catch(err => toast({ title: 'Hata', description: err || '', variant: 'destructive' }))
    }
  }
  ...
  >
    ...
    <TsnSelect title="Rol" list={getAdminRoleList()} value={member?.role} onValueChange={e => setMember({ ...member, role: e })} />
