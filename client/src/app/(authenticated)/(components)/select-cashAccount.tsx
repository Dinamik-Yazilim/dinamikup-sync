...
import { CashAccount, cashAccountListQuery } from "@/types/CashAccount"
...
interface Props {
  ...
  onSelect?: (e: CashAccount) => void
}
export function SelectCashAccount({ children, onSelect }: Props) {
  ...
  const load = (s?: string) => {
    setLoading(true)
    postItem(`/mikro/get`, token, { query: cashAccountListQuery({ search: s }) })
      .then(result => {
        setList(result as CashAccount[])
      })
      ...
      <AlertDialogContent className=" px-3 py-1 lg:max-w-[900px]">
        ...
        <div className="overflow-y-auto h-[600px]">
          ...
          <div className='grid grid-cols-1 w-full text-xs lg:text-sm border-b my-2 ps-2 pe-5'>
            <div className=''>CashAccount</div>
          </div>
          <div className="w-fu11ll overflow-y-auto h-[450px] ps-2 pe-2 lg:pe-4">
            {!loading && list && list.map((e: CashAccount, rowIndex) => <TsnDialogSelectButton key={'gridList-' + rowIndex}
...
export function SelectCashAccountWithLabel({ children, onSelect, className, caption }: WithLabelProps) {
  return (
    <div className={`w-full flex justify-between p-2 pe-4 items-start  border rounded-md border-dashed ${className}`}>
      <div className="flex flex-col gap-1">
        <Label className="text-muted-foreground">Kasa Hesabı</Label>
        <div className="capitalize">{caption}</div>
      </div>
      <SelectCashAccount onSelect={onSelect} ><ButtonSelect /></SelectCashAccount>
    </div>
  )
}